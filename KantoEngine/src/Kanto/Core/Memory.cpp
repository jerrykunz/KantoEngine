...
namespace Kanto
{
	...
	static Kanto::AllocationStats s_GlobalStats;
...
#ifndef KN_DIST
			if (!found)
				KN_CORE_WARN_TAG("Memory", "Memory block {0} not present in alloc map", memory);
#endif
		}
...
#if KN_TRACK_MEMORY && KN_PLATFORM_WINDOWS

_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
void* __CRTDECL operator new(size_t size)
{
	return Kanto::Allocator::Allocate(size);
}
...
void* __CRTDECL operator new[](size_t size)
	...
	{
		return Kanto::Allocator::Allocate(size, desc);
	}
	...
	_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
		...
		{
			return Kanto::Allocator::Allocate(size, file, line);
		}
		...
		_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
			...
			{
				return Kanto::Allocator::Free(memory);
			}
