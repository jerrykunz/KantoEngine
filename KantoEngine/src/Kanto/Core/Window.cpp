...
#include "Kanto/Embed/KantoIcon.embed"

	static void GLFWErrorCallback(int error, const char* description)
	{
		KN_CORE_ERROR_TAG("GLFW", "GLFW Error ({0}): {1}", error, description);
	}
	...
	void Window::Init()
	{
		...
		KN_CORE_INFO_TAG("GLFW", "Creating window {0} ({1}, {2})", m_Specification.Title, m_Specification.Width, m_Specification.Height);

		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			KN_CORE_ASSERT(success, "Could not intialize GLFW!");
		...
		if (!m_Specification.Decorated)
		{
			...
			//My guess is that titlebar is deprecated -J
...
//#endif
		...
		// Set icon
		{
			...
			{
				// Use embedded Kanto icon
				icon.pixels = stbi_load_from_memory(g_KantoIconPNG, sizeof(g_KantoIconPNG), &icon.width, &icon.height, &channels, 4);
				glfwSetWindowIcon(m_Window, 1, &icon);
				stbi_image_free(icon.pixels);
			}
		}
		...
		if (isRawMouseMotionSupported)
			...
			KN_CORE_WARN_TAG("Platform", "Raw mouse motion not supported.");
