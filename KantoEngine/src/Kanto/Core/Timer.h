...
namespace Kanto
{
	...
	{
	public:
		KN_FORCE_INLINE Timer() { Reset(); }
		KN_FORCE_INLINE void Reset() { m_Start = std::chrono::high_resolution_clock::now(); }
		KN_FORCE_INLINE float Elapsed() { return std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - m_Start).count() * 0.001f * 0.001f; }
		KN_FORCE_INLINE float ElapsedMillis() { return std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - m_Start).count() * 0.001f; }
	private:
	...
	public:
		...
		~ScopedTimer()
		{
			...
			KN_CORE_TRACE_TAG("TIMER", "{0} - {1}ms", m_Name, time);
		}
	private:
		std::string m_Name;
		Timer m_Timer;
