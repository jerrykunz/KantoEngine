...
//#include "Kanto/Editor/EditorConsole/EditorConsoleSink.h"

#include <filesystem>

#define KN_HAS_CONSOLE !KN_DIST
...
namespace Kanto 
{
	...
	void Log::Init()
	{
		...
		std::vector<spdlog::sink_ptr> KantoSinks =
		{
			std::make_shared<spdlog::sinks::basic_file_sink_mt>("logs/Kanto.log", true),
#if KN_HAS_CONSOLE
		...
		KantoSinks[0]->set_pattern("[%T] [%l] %n: %v");
		appSinks[0]->set_pattern("[%T] [%l] %n: %v");

#if KN_HAS_CONSOLE
		KantoSinks[1]->set_pattern("%^[%T] %n: %v%$");
		appSinks[1]->set_pattern("%^[%T] %n: %v%$");
		for (auto sink : editorConsoleSinks)
			sink->set_pattern("%^%v%$");
#endif

		s_CoreLogger = std::make_shared<spdlog::logger>("Kanto", KantoSinks.begin(), KantoSinks.end());
