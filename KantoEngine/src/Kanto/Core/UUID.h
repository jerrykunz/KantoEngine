...
namespace Kanto
{
	...
	// used to "uniquely" identify information. In Kanto, even though we use the term
...
namespace std {
	...
	template <>
	struct hash<Kanto::UUID>
	{
		std::size_t operator()(const Kanto::UUID& uuid) const
		{
	...
	template <>
	struct hash<Kanto::UUID32>
	{
		std::size_t operator()(const Kanto::UUID32& uuid) const
		{
