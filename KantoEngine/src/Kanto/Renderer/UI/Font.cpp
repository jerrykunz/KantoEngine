...
//#include "Kanto/Project/Project.h"
...
#define THREADS 8
	...
	static void CacheFontAtlas(const std::string& fontName, float fontSize, AtlasHeader header, const void* pixels)
	{
		...
		{
			stream.close();
			KN_CORE_ERROR_TAG("Renderer", "Failed to cache font atlas to {0}", filepath.string());
			return;
		}
	...
	static Ref<VulkanImage> CreateAndCacheAtlas(const std::string& fontName, float fontSize, const std::vector<GlyphGeometry>& glyphs, const FontGeometry& fontGeometry, const Configuration& config)
	{
		...
		/*VulkanImage* lol = new VulkanImage((const void*)bitmap.pixels,
			header.Width,
			header.Height,
			VulkanContext::Get()->PhysicalDevice->Device,
			*VulkanContext::Get()->Device,
			VulkanContext::Get()->CommandPool,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

		return nullptr;*/
		Ref<VulkanImage> texture = CreateRef<VulkanImage>((const void*)bitmap.pixels,
			header.Width,
			header.Height,
			VulkanContext::Get()->PhysicalDevice->Device,
			*VulkanContext::Get()->Device,
	...
	static Ref<VulkanImage> CreateCachedAtlas(AtlasHeader header, const void* pixels)
	{
		//return nullptr;

		Ref<VulkanImage> texture = CreateRef<VulkanImage>(pixels,
			header.Width,
			header.Height, 
			VulkanContext::Get()->PhysicalDevice->Device,
			*VulkanContext::Get()->Device,
	...
	/*	TextureSpecification spec;
		...
		spec.SamplerWrap = TextureWrap::Clamp;
		spec.DebugName = "FontAtlas";
		Ref<VulkanImage> texture = Texture2D::Create(spec, pixels);
		return texture;*/
	}
	...
	void Font::CreateAtlas(Buffer buffer)
	{
		...
		KN_CORE_ASSERT(glyphsLoaded >= 0);
		KN_CORE_TRACE_TAG("Renderer", "Loaded geometry of {0} out of {1} glyphs", glyphsLoaded, (int)charset.size());
		// List missing glyphs
		if (glyphsLoaded < (int)charset.size())
		{
			KN_CORE_WARN_TAG("Renderer", "Missing {0} {1}", (int)charset.size() - glyphsLoaded, fontInput.glyphIdentifierType == GlyphIdentifierType::UNICODE_CODEPOINT ? "codepoints" : "glyphs");
		}
		...
		atlasPacker.setInnerPixelPadding(config.imageType == ImageType::MSDF || config.imageType == ImageType::MTSDF ? 0 : -1);
		...
		if (int remaining = atlasPacker.pack(m_MSDFData->Glyphs.data(), (int)m_MSDFData->Glyphs.size()))
		{
			if (remaining < 0)
			{
				KN_CORE_ASSERT(false);
			}
			else
			{
				KN_CORE_ERROR_TAG("Renderer", "Error: Could not fit {0} out of {1} glyphs into the atlas.", remaining, (int)m_MSDFData->Glyphs.size());
				KN_CORE_ASSERT(false);
			}
		}
		atlasPacker.getDimensions(config.width, config.height);
		KN_CORE_ASSERT(config.width > 0 && config.height > 0);
		config.emSize = atlasPacker.getScale();
		//config.pxRange = atlasPacker.getPixelRange();
		config.pxRange = atlasPacker.getPixelRange().upper;
		if (!fixedScale)
			KN_CORE_TRACE_TAG("Renderer", "Glyph size: {0} pixels/EM", config.emSize);
		if (!fixedDimensions)
			KN_CORE_TRACE_TAG("Renderer", "Atlas dimensions: {0} x {1}", config.width, config.height);
	...
	void Font::Init()
	{
		...
		s_DefaultFont = CreateRef<Font>("Fonts/opensans/OpenSans-Regular.ttf");
		s_DefaultMonoSpacedFont = CreateRef<Font>("Fonts/SourceCodePro/static/SourceCodePro-Medium.ttf");
	}
