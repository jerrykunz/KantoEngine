...
#include "Kanto/Asset/AssetExtensions.h"
...
namespace Kanto 
{
	...
	{
		KN_PROFILE_FUNC();
		KN_SCOPE_PERF("AssetManager::GetAsset");
	...
	bool EditorAssetManager::ReloadData(AssetHandle assetHandle)
	{
		...
		if (!metadata.IsValid())
		{
			KN_CORE_ERROR("Trying to reload invalid asset");
			return false;
		}
	...
	{
		KN_CORE_INFO("[AssetManager] Loading Asset Registry");
		...
		if (!handles)
		{
			KN_CORE_ERROR("[AssetManager] Asset Registry appears to be corrupted!");
			KN_CORE_VERIFY(false);
			return;
		}
		...
		for (auto entry : handles)
		{
			...
			{
				KN_CORE_WARN_TAG("AssetManager", "Mismatch between stored AssetType and extension type when reading asset registry!");
				metadata.Type = GetAssetTypeFromPath(filepath);
			}

			if (!FileSystem::Exists(GetFileSystemPath(metadata)))
			{
				KN_CORE_WARN("[AssetManager] Missing asset '{0}' detected in registry file, trying to locate...", metadata.FilePath);
				...
				for (auto& pathEntry : std::filesystem::recursive_directory_iterator(Project::GetAssetDirectory()))
				{
					...
					if (bestScore > 0)
						KN_CORE_WARN("[AssetManager] Multiple candidates found...");
					...
					KN_CORE_WARN("'{0}' has a score of {1}, best score is {2}", path.string(), score, bestScore);
				...
				{
					KN_CORE_ERROR("[AssetManager] Failed to locate a potential match for '{0}'", metadata.FilePath);
					continue;
				}
				...
				KN_CORE_WARN("[AssetManager] Found most likely match '{0}'", metadata.FilePath);
			}

			if (metadata.Handle == 0)
			{
				KN_CORE_WARN("[AssetManager] AssetHandle for {0} is 0, this shouldn't happen.", metadata.FilePath);
				continue;
			}
		...
		KN_CORE_INFO("[AssetManager] Loaded {0} asset entries", m_AssetRegistry.Count());
	}
	...
	void EditorAssetManager::WriteRegistryToFile()
	{
		...
		KN_CORE_INFO("[AssetManager] serializing asset registry with {0} entries", sortedMap.size());
